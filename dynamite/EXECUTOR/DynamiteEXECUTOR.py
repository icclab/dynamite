...
class DynamiteEXECUTOR(Process):
    ...
    _name_scaling_request_queue = "dynamite_scaling_request"
    _name_scaling_response_queue = "dynamite_scaling_response"

    _rabbit_mq_connection = None
    _rabbit_mq_channel = None
    _rabbit_mq_endpoint = "127.0.0.1:5672"
    _rabbit_mq_endpoint_host = None
    _rabbit_mq_endpoint_port = None
    ...
    def __init__(self,
        ...
        if rabbit_mq_endpoint is not None and isinstance(rabbit_mq_endpoint, str):
            self._rabbit_mq_endpoint = rabbit_mq_endpoint

        ip_port = self._rabbit_mq_endpoint.split(":")
        self._rabbit_mq_endpoint_host = ip_port[0]
        self._rabbit_mq_endpoint_port = int(ip_port[1])

        if name_scaling_request_queue is not None:
            if isinstance(name_scaling_request_queue, str):
                self._name_scaling_request_queue = name_scaling_request_queue
            else:
        ...
        if name_scaling_response_queue is not None:
            if isinstance(name_scaling_response_queue, str):
                self._name_scaling_response_queue = name_scaling_response_queue
            else:
    ...
    def __repr__(self):
        return "DynamiteEXECUTOR(" \
               "rabbit_mq_endpoint=\"{}\", " \
               "etcd_enpoint=\"{}\", " \
               "name_scaling_request_queue=\"{}\", " \
               "name_scaling_response_queue=\"{}\", " \
               .format(
                    self._rabbit_mq_endpoint,
                    self._etcd_endpoint,
                    self._name_scaling_request_queue,
                    self._name_scaling_response_queue,
               )

    def _create_rabbit_mq_connection(self):
        rabbit_mq_host = self._rabbit_mq_endpoint_host
        rabbit_mq_port = self._rabbit_mq_endpoint_port
    ...
    def _send_scaling_response_to_rabbitmq_response_queue(self, scaling_response):
        ...
        self._logger.debug("Sending scaling response to rabbitmq response queue:{:s}".format(scaling_response_string))

        if self._rabbit_mq_channel:
            self._rabbit_mq_channel.basic_publish(exchange='',
                                                 routing_key=self._name_scaling_response_queue,
    ...
    def run(self):
        ...
        if self._rabbit_mq_channel:
            self._rabbit_mq_channel.basic_consume(self._scaling_request_received,
                                                 queue=self._name_scaling_request_queue,
                                                 no_ack=False)

            self._rabbit_mq_channel.start_consuming()
